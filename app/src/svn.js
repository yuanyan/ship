...
svn.run = function (cmd, args, callback) {
    ...
    var proc = spawn(cmd, args, { cwd: this.repoRoot });

    this.emit("cmd", proc, cmd, args);
    console.warn("Running cmd: ", cmd, args);

    proc.stdout.on('data', function (chunk) {
        outBufferList.push(chunk);
    });

    proc.stderr.on('data', function (data) {
         errString += iconv.decode(data,'gbk');
        //ssh warning, ignore
        if (errString.indexOf("Killed by signal 15.") === -1) {
    ...
    proc.on('close', function (code) {
        if (callback) {
            var outBuffer = Buffer.concat(outBufferList);
            // decode chinese encoding
            var outString = iconv.decode(outBuffer,'gbk');
            callback(errString, outString);
        }
    });
